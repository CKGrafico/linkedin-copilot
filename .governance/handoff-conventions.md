...
> **Status: ACTIVE** — Defines how agents pass outputs to each other. Maintained by rcontributorr.
...
1. The **receiving agentrs file** (ragents/receiving-agent.mdr)
...
**Trigger:** rsourcerr has identified a specific person worth contacting.

**What to pass:**
- Session note including: personrs name, role, company, rationale, and any observed personalization hooks
- ragents/outreach-writer.mdr
- r.agents/skills/outreach-drafting/SKILL.mdr
- r.agents/skills/message-personalization/SKILL.mdr

**Expected input format:** The note entry for the person, formatted per rnote-takingr skill.

---

### Headhunter → Researcher (future)

**Trigger:** rheadhunterr has identified a company or role worth investigating in depth.

**What to pass:**
- Session note including: company name, role, initial fit assessment, and open questions
- ragents/researcher.mdr
- r.agents/skills/company-research/SKILL.mdr
...
**What to pass:**
- Filled-in rtemplates/contributor-request.mdr
...
- An agent must never assume it has access to another agentrs internal state. Every session starts fresh from files and notes.
- If a handoff requires information that was not captured in the session note, the previous agentrs session must be resumed or reconstructed.
- Handoff rules for each agent are also documented in the agentrs own **Handoff Rules** section.

---

## What agents must not hand off

- Direct instructions to edit repo files. Route all repo changes through rcontributorr.
